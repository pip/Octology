# 2AFBQB7 - fULnAME created by Pip@CPAN.Org to simplify Perl text-mode application development
...
my @DISPSTAK = ( );# global stack of created pKGnAME objects for display order
...
sub TIEARRAY {
  OScr() unless($GLBL{'FLAGOPEN'}); # need Open main Screen for new pKGnAME obj
  my $clas = shift;
  my $self = bless({}, $clas);
  ...
  while(@_) {
    ...
    unless($foun) {
      ...
      unless($foun) {
        croak "!*EROR*! fULnAME::new initialization key:$keey was not recognized!\n";
      }
    }
  }
  ...
  if($curs) {
    ...
    unless(exists($self->{'_wind'}) && defined($self->{'_wind'})) {
      exit();
#      croak "!*EROR*! fULnAME::new could not create new window with hite:$self->{'_hite'}, widt:$self->{'_widt'}, yoff:$self->{'_yoff'}, xoff:$self->{'_xoff'}!\n";
    }
  }
  ...
  # add new pKGnAME object to display order stack
  $self->{'_dndx'} = @DISPSTAK;
  push(@DISPSTAK, \$self);
  return($self);
}
...
# MkMethdz creates pKGnAME object field accessor methods with
    ...
    *{$meth} = sub {
      ...
      while(@_) {
        ...
        if     ($keey =~ /\d+$/) { # call a special sub for numeric keyz
          ...
          unless($foun) {
            croak "!*EROR*! fULnAME::$meth key:$keey was not recognized!\n";
...
#  no strict 'subs';
  ...
  if($GLBL{'FLAGCOLR'}) {
    ...
    if(defined($self->{'_wind'})) {
      ...
      if($bold) { $self->{'_wind'}->attron( $abld); }
      else      { $self->{'_wind'}->attroff($abld); }
...
sub BordChar { # return characters for different border types
  ...
  unless($noip) { # No InitPair flag to say border color staying same
    my $fgcl = $self->{'_brfc'};
    my $bgcl = $self->{'_brbc'};
    $self->InitPair($fgcl, $bgcl) if($self->{'_flagclru'});
  }
  $self->{'_wind'}->addch( $BORDSETS[($self->{'_btyp'} - 1)]{lc($loca)} );
}

sub CnvAnsCC { # convert ANSI escape color codes into pKGnAME color codes
  my $self = shift; my $acod = shift;
  my @alut = @telc;
  my $bold = 0; my($fgcl, $bgcl)=('w', 'k');
    ...
    if(defined($valu)) {
      ...
      unless($foun) {
        if($keey =~ /slvl/i) {
          ...
          croak "!*EROR*! fULnAME::ShokScrn key:$keey was not recognized!\n";
#        $keey =~ s/^_*/_/; # auto-add unfound
#        $self->{$keey} = $valu;
        }
      }
...
sub OScr { # Open a new Curses Screen && setup all useful stuff
  ...
  unless($GLBL{'FLAGOPEN'}) {
    $GLBL{'FLAGOPEN'} = 1;
    if(!$curs && $^O eq 'MSWin32' && lc($ENV{'COMSPEC'}) eq 'c:\pip\4nt\4nt.exe') {
      $GLBL{'FLAGU4NT'} = 1; # clear 4NT screen && get variables
      my $data = `echos  _KBHIT   %_KBHIT%;   &
                  ...
                  echos  _SECOND  %_SECOND%;`;
      while($data =~ s/(\w+)\s+([^;]*);//) { $SDAT{$1} = $2; }
      return;
    }
    initscr(); noecho(); nonl(); raw();
...
#     O_BS_OVERLOAD          );
    ...
    # add my own new additional key<->num mappings (i.e., 265..279 => F1..F15)
    for($i = 265; $i <= 279; $i++) { $knum{"$i"} = "KEY_F" . ($i-264); }
    # find the right value of the A_BOLD attribute so strict doesn't complain
    for($i = 0; $i < @kndx; $i++) {
      if(defined($knam[$i]) && $knam[$i] eq 'A_BOLD') {
...
sub Prnt { # pKGnAME object PrintString method
  ...
  # if text, fclr, or bclr are arrays like new or Draw would take, join them
  $chrz = ref($parm{'text'});
  ...
  if(exists($parm{'fclr'}) || exists($parm{'bclr'})) {
    if($parm{'prin'}) {
      if($self->{'_btyp'}) {
        if($parm{'ycrs'}) { $parm{'ycrs'}--; } else { $parm{'zery'} = 1; }
        if($parm{'xcrs'}) { $parm{'xcrs'}--; } else { $parm{'zerx'} = 1; }
    ...
    $cnum = 0;
    while(length($parm{'text'})) {
      $chrz = substr($parm{'text'}, 0, 1, ''); $delt = 0;
      if(exists($parm{'fclr'}) && length($parm{'fclr'})) {
        $fgcl = substr($parm{'fclr'}, 0, 1, '');
      }
      if(exists($parm{'bclr'}) && length($parm{'bclr'})) {
        $bgcl = substr($parm{'bclr'}, 0, 1, '');
      }
      $self->InitPair($fgcl, $bgcl);
      while((!exists($parm{'fclr'}) ||
             !length($parm{'fclr'}) ||
              substr($parm{'fclr'}, 0, 1) eq $fgcl) &&
            (!exists($parm{'bclr'}) ||
             !length($parm{'bclr'}) ||
              substr($parm{'bclr'}, 0, 1) eq $bgcl) &&
              length($parm{'text'})) {
        $cnum++; $delt++;
        substr($parm{'fclr'}, 0, 1, '') if(exists($parm{'fclr'}) && length($parm{'fclr'}));
        substr($parm{'bclr'}, 0, 1, '') if(exists($parm{'bclr'}) && length($parm{'bclr'}));
        $chrz .= substr($parm{'text'}, 0, 1, '');
      }
      ...
      if(exists($parm{'ycrs'}) && exists($parm{'xcrs'})) {
        if($curs) {
          ...
          my $scrp = "\@scrput " . $parm{'ycrs'} . ' ' . $parm{'xcrs'} + ($cnum - $delt) . ' ';
          ...
          if(defined($fgct) && defined($bgct)) {
            ...
            $scrp = "\@scrput " . $parm{'ycrs'} . ' ' . $parm{'xcrs'} + ($cnum - $delt) . ' ';
          }
          $scrp .= $chrz;
          print SCRP "$scrp\n";
        }
    ...
    if(exists($parm{'ycrs'}) && exists($parm{'xcrs'})) {
      if($curs) {
        ...
        print SCRP "\@screen " . $parm{'ycrs'} . ' ' . $parm{'xcrs'} . ' ' . $parm{'text'} . "\n";
      }
    }
  }
  ...
  if($curs) {
    ...
    close(SCRP); system("call C:\\SimpDraw.bat");
  }
  return($cnum);
}

sub Draw { # pKGnAME object self Drawing method
  ...
  while(@_) { # load key/vals like new()
    ...
    if(defined($valu)) {
      ...
      unless($foun) {
        exit;
#        croak "!*EROR*! fULnAME::Draw key:$keey was not recognized!\n";
#        $keey =~ s/^_*/_/; # auto-add unfound
#        $self->{$keey} = $valu;
      }
    } else {
    ...
    if(defined($valu)) {
      ...
      unless($foun) {
        if($keey =~ /wait/i) {
          ...
          croak "!*EROR*! fULnAME::Wait key:$keey was not recognized!\n";
#        $keey =~ s/^_*/_/; # auto-add unfound
#        $self->{$keey} = $valu;
        }
      }
    ...
    if(defined($valu)) {
      ...
      unless($foun) {
        if     ($keey =~ /tmot/i || $keey eq 'Timeout') {
          ...
          exit;
#          croak "!*EROR*! fULnAME::GetK key:$keey was not recognized!\n";
#        $keey =~ s/^_*/_/; # auto-add unfound
#        $self->{$keey} = $valu;
        }
      }
  ...
  foreach(@KMODNAMZ) { $self->{'_kmod'}->{$_} = 0; } # clear keymods
...
#print "\nchar:$char ordc:$ordc ord:" . ord($char) . "\n";
  ...
  foreach(@KMODNAMZ) { $self->{'_mque'}->[0]->{$_} = $self->{'_kmod'}->{$_}; }
    ...
    if(defined($valu)) {
      ...
      unless($foun) {
        if($keey =~ /kmod/i) {
          ...
          exit;
#          croak "!*EROR*! fULnAME::KMod key:$keey was not recognized!\n";
#        $keey =~ s/^_*/_/; # auto-add unfound
#        $self->{$keey} = $valu;
        }
      }
...
sub Updt { # update a pKGnAME object's dimensions (resize && mvwin)
    ...
    if(defined($valu)) {
      ...
      unless($foun) {
        if($keey =~ /noch/i) {
          ...
          croak "!*EROR*! fULnAME::Updt key:$keey was not recognized!\n";
#        $keey =~ s/^_*/_/; # auto-add unfound
#        $self->{$keey} = $valu;
        }
      }
...
# Mesg() is a special fULnAME object constructor which creates a
...
sub Mesg {
  ...
  unless(exists($self->{'_wind'}) && defined($self->{'_wind'})) {
    exit;
#    croak "!*EROR*! fULnAME::Mesg could not create new window with hite:$self->{'_hite'}, widt:$self->{'_widt'}, yoff:$self->{'_yoff'}, xoff:$self->{'_xoff'}!\n";
  }
...
# Prmt() is a special fULnAME object constructor which creates a
...
#  $self->{'_dref'} = \$data;        # default text data ref !exist at start
  ...
  $self->{'_wind'} = newwin($self->{'_hite'}, $self->{'_widt'},
                            $self->{'_yoff'}, $self->{'_xoff'});
  unless(exists($self->{'_wind'}) && defined($self->{'_wind'})) {
    croak "!*EROR*! fULnAME::Prmt could not create new window with hite:$self->{'_hite'}, widt:$self->{'_widt'}, yoff:$self->{'_yoff'}, xoff:$self->{'_xoff'}!\n";
  }
  $self->FlagCVis(); # set cursor visibility to new object state
  $self->TestDraw();
  $self->{'_dndx'} = @DISPSTAK; # add object to display order stack
...
# Focu() is a fULnAME method which give focus to special
...
# CPik() is a special fULnAME object constructor which creates a
#   Color Pick window.
# If params are supplied, they must be hash key => value pairs.
sub CPik {
...
#  $self->{'_hite'} = getmaxy() - 4; #                     && high
  ...
  $self->{'_wind'} = newwin($self->{'_hite'}, $self->{'_widt'},
                            $self->{'_yoff'}, $self->{'_xoff'});
  unless(exists($self->{'_wind'}) && defined($self->{'_wind'})) {
    croak "!*EROR*! fULnAME::CPik could not create new window with hite:$self->{'_hite'}, widt:$self->{'_widt'}, yoff:$self->{'_yoff'}, xoff:$self->{'_xoff'}!\n";
  }
  $self->FlagCVis(); # set cursor visibility to new object state
...
# Brws() is a special fULnAME object constructor which creates a
...
#$self->{'_eflz'}->{$_} = 0 foreach('_cnfg','_mkdr','_cdup','_help','_filt');
  ...
  $self->{'_wind'} = newwin($self->{'_hite'}, $self->{'_widt'},
                            $self->{'_yoff'}, $self->{'_xoff'});
  unless(exists($self->{'_wind'}) && defined($self->{'_wind'})) {
    croak "!*EROR*! fULnAME::Brws could not create new window with hite:$self->{'_hite'}, widt:$self->{'_widt'}, yoff:$self->{'_yoff'}, xoff:$self->{'_xoff'}!\n";
  }
  $self->{'_dndx'} = @DISPSTAK; # add object to display order stack
  push(@DISPSTAK, \$self);
...
fULnAME - wrappers to make Curses application development Simple
...
fULnAME provides a curt mechanism for updating a console screen
...
=item - consider breaking sub (CPik|Brws|.+?) into own fULnAME::$1.pm
          instead of letting pKGnAME.pm remain so cluttered
...
An example of setting and updating 'WindowHeight':
  ...
  use fULnAME;
  my $simp = fULnAME->new( 'WindowHeight' => 7 ); # set
...
Most other fULnAME methods also accept hash key => value pairs as
...
update many pKGnAME fields with a call to any particular
...
fULnAME now supports tied array interfaces as the new preferred
...
for printing colors), here is an example of how it can be done:
  ...
  my $simp = tie(@text, 'fULnAME');
             tie(@fclr, 'fULnAME::FClr', $simp);
             tie(@bclr, 'fULnAME::BClr', $simp);
...
Returns the pKGnAME form of the ANSI color code
$AnsiColorCode.
...
to the fULnAME package but I have exposed it because it could
be useful elsewhere.

=head2 ShokScrn or ShockScreen( [$FlagClear] )

ShockScreen() forces the screen and all created pKGnAME objects
...
Returns the current pKGnAME object's window height
(last index: Height() - 1)

=head2 Widt or Width

Returns the current pKGnAME object's window width
(last index: Width() - 1)
...
Draws the current pKGnAME object with the established TextData() and
ColorData() functions.

DrawWindow() accepts a hash of parameters like new() which will update
as many attributes of the pKGnAME object as are specified by key => value
pairs.
...
MoveCursor() updates the current pKGnAME object's cursor position
...
ResizeWindow() updates the current pKGnAME object's window dimensions
...
This can be used to make simple one-character prompt windows.  For example:
  ...
  use fULnAME;
  my $simp   = fULnAME->new();
  my $answer = $simp->MessageWindow('titl' => 'Is pKGnAME useful?',
                                    'pres' => '(Yes/No)');
...
DeleteWindow() deletes all the components of the created pKGnAME object
and calls ShockScreen() to cause the screen and all other created
objects to be redrawn.

=head1 ACCESSOR AND FLAG METHODS

pKGnAME accessor and flag object methods have related interfaces as they
each access and update a single component field of fULnAME objects.  Each
one always returns the value of the field they access.  Thus if you want
to obtain a certain value from a pKGnAME object, just call the accessor
...
=head2 Flag Accessors
  ...
  FlagADTF or FlagAutoDrawTiedForegroundData 1 # Automatic DrawWindow() call
    #   for arrays tied to fULnAME::FClr objects when FlagAuDr is already set
  FlagADTB or FlagAutoDrawTiedBackgroundData 1 # Automatic DrawWindow() call
    #   for arrays tied to fULnAME::BClr objects when FlagAuDr is already set
...
=head2 Accessor and Flag Method Usage Examples
  ...
  my $simp = fULnAME->new('text' => [ 'hmmm', 'haha', 'whoa', 'yeah' ],
