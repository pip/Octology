# 524DkqX - fULnAME.pm    created by Pip Stuart     <Pip@CPAN.Org>, derived from Tk::WorldCanvas (by Joseph Skrovan <Joseph@Skrovan.Com>
#    which was based on a version by Rudy Albachten <Rudy@Albachten.Com>)     && Tk::RotCanvas   (by Ala Qumsieh    <AQumsieh@CPAN.Org>).
# pKGnAME provides an alternative to Tk::Canvas which can zoom, pan, && rotate.
...
fULnAME - Canvas with Abstract center, zoom, && rotate methods
...
=head1 SYNOPSIS
  ...
  my $acnv = $mwin->pKGnAME()->pack('-expand' => 1,
    '-fill'  =>  'both');
  ...
  my $line   = $acnv->createLine(      0,  1, 31, 32,
    '-fill'  =>  'blue',
    '-arrow' =>  'last');
  my $labl   = $mwin->Label('-text' => 'Hello pKGnAME! =)');
...
pKGnAME provides an alternative to a L<Tk::Canvas> object which partially abstracts the coordinates of objects drawn onto itself.  This allows the
...
fULnAME is derived from the excellent modules L<Tk::WorldCanvas> by Joseph Skrovan <Joseph@Skrovan.Com> (which was itself based on a version by Rudy
...
I<pKGnAME> is meant to be a useful alternative to a regular Canvas.  Typically, you should call $acnv->viewAll() (or
$acnv->viewArea(@box)) before calling MainLoop().

Most of the I<pKGnAME> methods are the same as regular I<Canvas> methods except that they accept && return abstract coordinates instead of widget coordinates.

I<pKGnAME> also adds a new rotate() method to allow rotation of canvas objects by arbitrary angles.
...
Zooms the display by the specified amount.  Example:
  ...
  my $acnv = $mwin->Scrolled('pKGnAME', -scrollbars => 'nw',); # ...
  #   you want to bind the key-presses to the 'pKGnAME' Subwidget of Scrolled.
...
If you do want to change the arrow key-bindings to pan in abstract coordinates using B<panAbstract> you must disable the default arrow key-bindings.  Example:
  ...
  $mwin->bind('pKGnAME',    '<Up>' => '');
  $mwin->bind('pKGnAME',  '<Down>' => '');
  $mwin->bind('pKGnAME',  '<Left>' => '');
  $mwin->bind('pKGnAME', '<Right>' => '');
...
whole pKGnAME, 2 pans, && 3 zooms.  The default of this value is 0 but this option is very useful if you don't need Control-Button bindings for some
...
Returns the scale value of the pKGnAME relative to the underlying canvas.  An optional parameter can be supplied to set the value although the zoom
...
The band color is set with the I<pKGnAME> option '-bandColor'.  The default color is 'red'.  Example specifying a region to delete:
...
This box avoids the overhead of bounding box calculations that can occur if you create your own rubberBand outside of I<pKGnAME>.

=item I<$acnv>->B<viewAll>([-border => number])

Displays at maximum possible zoom all objects centered in the I<pKGnAME>.  The switch '-border' specifies, as a percentage of the screen, the minimum
...
Displays at maximum possible zoom the specified region centered in the I<pKGnAME>.

=item I<$acnv>->B<viewFit>([-border => number], I<TagOrID>, [I<TagOrID>, ...])

Adjusts the pKGnAME to display all of the specified tags.  The '-border' switch specifies (as a percentage) how much extra surrounding space should be shown.
...
Abstract coordinates are supplied && returned to B<pKGnAME> methods instead of widget coordinates unless otherwise specified.  (i.e., These methods take
...
I<Tk::pKGnAME> option '-changeView' can be used to specify a callback for a change of the view area.  This is useful for updating a second pKGnAME which
is displaying the view region of the first pKGnAME.
...
specifed by the user calling 'configure'.  Example:
  ...
  # viewAll if 2nd pKGnAME widget is resized.
...
I<fULnAME> sets '-confine => 0' by default to avoid these problems.  You can change it back with:

  $acnv->configure(-confine => 1);

(2) '-scrollregion' is maintained by I<pKGnAME> to include all objects on the canvas.  '-scrollregion' will be adjusted automatically as objects are
...
Copyright: (c) 2005-2007, Pip Stuart.
...
pKGnAME is derived from code by:
