# 3159mLT - fULnAME.pm created by Pip Stuart <Pip@CPAN.Org> to CoNVert between arbitrary number Bases.  I'm totally addicted to bass!
...
# only export cnv() for 'use fULnAME;' && all other stuff optionally
our @EXPORT      =             qw(cnv                                    )    ;
...
diginit(); # initialize the Dflt digit set whenever pKGnAME is used
127;
=head1 NAME

fULnAME - fast functions to CoNVert between number Bases
...
pKGnAME provides a few simple functions for converting between arbitrary number bases.  It is as fast as I currently know how to make it (of course
...
The reason I created pKGnAME was that I needed a simple way to convert quickly between the 3 number bases I use most (10, 16, && 64).  It turned out
...
pKGnAME can simply && realiably convert between any possible base between 2 && 64 (or 85).  I'm happy with it && employ b64() in places I probably
shouldn't now =).
...
cnv() is the only function that is exported from a normal 'use fULnAME;' command.  The other functions below can be imported to local namespaces
...
little as is necessary by default. So to be responsible, I have limited pKGnAME exporting to only cnv() under normal circumstances.  Please
...
I hope you find fULnAME useful.  Please feel free to e-mail me any suggestions or coding tips or notes of appreciation ("app-ree-see-ay-shun").
Thank you.  TTFN.
