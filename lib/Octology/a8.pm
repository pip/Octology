...
our @EXPORT= qw(bfr8colr b8colr d8colr dur8colr    d8cs @d8cl  chti  c8fn     S2   c2  S c   sS    lodl @Monz @Mon     %mc2F %mc2b %mF2c %mb2c        %sb10
...
our $VERSION='0.0';my  $d8VS='H9NM3tLi';our $auth='PipStuart <Pip@CPAN.Org>'; # above not exporting $b since collidez with sort{$a  <=> $b}
...
our %mc2F;our %mc2b;our %mF2c;our %mb2c; # MapC8Col8ColrCodz2ForeBackgrnd && rEverse
if(exists($ENV{'TERM'}) && $ENV{'TERM'} !~ /^(([Ex]|st)term|screen|rxvt)/){$tnhf=1;} # TermNoHandleFlag
...
"   S  - produce eScape codes from c8 Fbs     Vers:$VERSION  d8VS:$d8VS  Auth:$auth
        ...
        `c` should be the inverse function.
  -h  - print this Help text and exit;";return('');}
    ...
    #f($#sepc > 1   && $sepc[2]=~  /^[0-9A-Z._]$/i      ){$Sstr.= $SKpf . $sepc[2] . $SKpt;} # will need to add a -f flag l8r 2 design8 f0nt layer
  } return($Sstr);}
...
sub   d8colr {my $rtns = ''; # maybe these should eventually accept flags to select among most useful coloring options
...
sub dur8colr {my $rtns = ''; # should build     dur8ion colors right-to-left like b8colr but reversing the d8bo color order
  if(!@_ && !-t STDIN){chomp(@_=decode('UTF-8',<STDIN>));}
  for my $d8st (@_){my @d8di = split(//,reverse($d8st));my $d8dc = '';for(0..$#d8di){$d8dc  = $d8cl[$#d8cl - ($_%@d8cl)] . $d8di[$_] . $d8dc;} $rtns.="$d8dc ";
  }      $rtns =~ s/\s$//;return($rtns);}
sub bfr8colr {my $rtns = ''; # should build b8 fraction colors left-to-right like d8colr but reversing the d8bo color order
...
sub   b8colr {my $rtns = ''; # just do coloring stringwise for when sprintf zero-padding a specific width is needed   # mk b8clr 2Dtect commaz && % B4 coloring
  if(!@_ && !-t STDIN){chomp(@_=decode('UTF-8',<STDIN>));}
...
#   'to' => 56, 'from' => 28, 'convert' => 22, 'utility' => 22,  'base' => 13, 'decimal' => 6, 'XML' => 6, 'NotYetInModule' => 6; # these R probably top8 4now
  ...
  my %bfst=( # BinFileSummaryText
    ...
    'b8clr'    => "a8  colorizer for  b8  b64 which  handles fractions and commas         ",
    ...
    'c'        => "a8 *converter from ANSI escape codes to c8 (col8) format   (Fbs out)   ",
    ...
    'ckm8'     => "c8  NotYetInModule,textual Chess applic8ion in SDL (or Curses::Simp)   ",
    'cma'      => "b8  CoMmA inserting utility for every 3 digit characters (/[0-9]+/ )   ",
    'cnv'      => "b8  Math::BaseCnv CoNVerter,try2autodetect intended base (like `b8`)   ", # mAB rEtIr orig cnv once b8 is thoroughly reliable instead
    'coma'     => "b8  COMmA inserting utility for every 4  b64  chars (/[A-Z._0-9]+/i)   ",
    ...
    'd8'       => "d8 *DateTime stamps and conversion   (-a ANSI color, -f Full format)   ",
    'd8colr'   => "a8  colorizer for d8 (Date and Time) stamps    (Left -to-Right 4wrd)   ",
    'd8g'      => "d8  DateTime stamp utility for bookending e-mail with Gerry (my Dad)   ",
    ...
    'dic'      => "b8  wrapper for `dict`(ionary) command (similar to piping2`colorit`)   ",
    'die8'     => "c8  pretty terse dice-rolling  utility (similar to HTTP://Rolz.Org )   ",
    'dm2u'     => "a8 *converts Dos or Mac line-endings to Unix   (-s double to Single)   ",
    ...
    'frip'     => "    NotYetInModule,`flac` lossless CD audio 'RIPper' (AKA:extractor)   ",
    ...
    'lodH'     => "a8  load ~/.Hrc into d8a-structure and print in color    (like `a8`)   ", # mAB rEtIr once a8 struct&&color becomes more capable
   ...
   # pimp cryptic name:Pips Intergallactive Moosex Plaqueluster
    'pimp'     => "    PipsInteractiveMusicPlaylister,old C::Simp shuffled audio player   ", # mAB rEtIr sinc g3 is alreD gr8 (but consider rand playlistz)
    ...
    'rotW'     => "b8  utility to rot8 each character of passed in b64  number-strings    ",
    ...
    'tiglet'   => "f8  basic interactive selection of desired `figlet` font and text      ", # proly rEwrIt with Curses IF
    'tsgr'     => "c8  Test SelectGraphicRendition escapes && xterm-256color Blocks(-b)   ", # mAB shud mv2 a8 lIk tstc?
    'tst'      => "    directory where I experiment with new candid8 binary executables   ",
    'tstc'     => "a8 *TeST eScapes `S` of c8 `c` format for pal8 colors and attributes   ", # mAB fold in2 tsgr && mv latr2 a8,or mk -blox in tstc?
    'U2b2'     => "U8  download from YouTube just audio in high-quality    (auto-best?)   ",
...
#   'readhead' => "    read header inform8ion out of binary d8a files (like image size)   ", # rEtIrd as just Utl:rdhd() (mAB mk as shL func2?)
 ...
 # H4NM4NM4:keeping to exactly 1 160x50 page contains barely worthwhile:c8,curs,mix8,pimp,pmix,prym,readhead,shl8 so try2upd8 them to be useful or retire them;
  my $bfle=$#bfls;my %bftc=('grup'=>{'a8'=>'F:A','b8'=>'F:B','c8'=>'F:C','d8'=>'F:R','f8'=>'F:o','m8'=>'F:M','U8'=>'F:Y', # BinFilzListEnd && TypeCodez
     ...
     'strings'=>'F:8','ANSI'=>'ob', ',' =>'Lk','bfr8'=>'F:B','col8'=>'F:C','dur8'=>'F:R',      '\('=>'Wn','\)'=>'Wn','\['=>'ob','\]'=>'ob',
  ...
  for (0..$bfle){my $grup='';$grup='dirz' if(-d "$ENV{'HOME'}/bin/$bfls[$bfle - $_]");
    if( exists($bfst{$bfls[$bfle - $_]})){
      if($bfst{$bfls[$bfle - $_]}=~  /^\s*(\S+)/){$grup=$1 unless($grup eq 'dirz');$i=$j; # look for group to match Xpected as always 1st non-space string
       ...
       #$l=S($bftc{'file'}{ '\]' });$bfst{$bfls[$bfle-$_]}=~ s/([\]]+)/$l$1$i/gx; # Xplicitly do brak Rite
        $h=S($bftc{'file'}{ '!'  });$bfst{ $bfls[$bfle - $_]}=~ s/(\W|n|m)(!)/$1$h$2$i/g;d8cs('oCRoMoGY' ); # tryng some non-d8bo colrsequ for Octology too
        $h=d8colr('Octology'      );$bfst{ $bfls[$bfle - $_]}=~ s/Octology   /$h$i/gx   ;d8cs('CoMoR'    ); # colr all bangz 1st thng,thN all"color",cnv,&& utl
        $h=d8colr('color'         );$bfst{ $bfls[$bfle - $_]}=~ s/([^`])color/$1$h$i/g  ;d8cs('XXXNNN111'); # alsObAsic2pal8regXzBlO du!Use any $h or $l 2lFt
        $h=d8colr('converter'     );$bfst{ $bfls[$bfle - $_]}=~ s/converter  /$h$i/gx   ;d8cs('ZC9TVmM'  ); # rEmMbr2rEset2 d8bo whN done BlO
        $h=d8colr('utility'       );$bfst{ $bfls[$bfle - $_]}=~ s/utility    /$h$i/gx   ;d8cs('mmmMMMppVVVVVV'); # NYIM was orig S('mkB') at very end
        ...
        $d=S($bftc{'modu'}{'Games::Cards::Poker'});$bfst{$bfls[$bfle-$_]}=~ s/(Games::Cards::Poker)/$d$1$i/gx;  # try2get module nAm @stRt in 'no' grup 
        $d=S($bftc{'b64r'}{'\['});
        $e=S($bftc{'b64r'}{ '-'});
        $f=S($bftc{'b64r'}{ '0'});
        $h=S($bftc{'b64r'}{ '9'});
        ...
        $bfst{ $bfls[$bfle - $_]}=~ s/  ( base  )(128 )(\s   )/$1$d$2$i$3/gx        ;$d=S($bftc{'base'}{ '256'  }); # du 128
        ...
        $bfst{ $bfls[$bfle - $_]}=~ s/   (\s)   (Cards)(\s   )/$1$d$2$i$3/gx        ;$d=S($bftc{'file'}{ 'SKpz' }); # just get b128 Cards, not Gmz:Cards:Pokr
        ...
        $bfst{ $bfls[$bfle - $_]}=~ s/   (\s)   (down)(load(ed)?)/$1$d$2$l$3$i/gx   ;
    ...
    }else{if($sunf){$bfls[$bfle - $_] .= ' ' x (80 - length($bfls[$bfle - $_]));push(@ufls,$bfls[$bfle - $_]);} # ShowUndescribedFiles saved for l8r
  ...
  my   @bgls=qw(a8 b8 c8 d8 Time::DayOfWeek Time::PT f8 m8 U8 Utl none dir);my %bgml=();for(@bgls){$bgml{$_} =   [];} # init BinGroupLiSt && BinGroupMemberList
  ...
  if   ($htxt){print  $out8 $htxt;}
    ...
    if(@bfls % 2){ # tried altern8ing dark blue bkgr, but too stripey && $O was resetting but should be as hard now
      for (0 ..         $blih  ){
        if(-d "$ENV{'HOME'}/bin/$bfls[$_]"){$dirn= sprintf("%-9s",$bfls[$_].'/');$dirn=~ s/(\/)/$f$1$d/g; # 4m@ && colr dir key
                                   printf($out8 "$d%s"  ,$dirn            );print $out8 $bfst{$bfls[$_        ]} if(exists($bfst{$bfls[$_        ]}));
        }else{                     printf($out8 "$h%-9s",$bfls[$_        ]);print $out8 $bfst{$bfls[$_        ]} if(exists($bfst{$bfls[$_        ]}));
        }
        if(-d "$ENV{'HOME'}/bin/$bfls[$_]"){$dirn= sprintf("%-9s",$bfls[$_].'/');$dirn=~ s/(\/)/$f$1$d/g; # 4m@ && colr dir key
          if(@bfls > $_+$blih+1){  printf($out8 "$d%s"  ,$dirn            );print $out8 $bfst{$bfls[$_+$blih+1]} if(exists($bfst{$bfls[$_+$blih+1]}));
                                   say    $out8  $z;}
        }else{
          if(@bfls > $_+$blih+1){  printf($out8 "$h%-9s",$bfls[$_+$blih+1]);print $out8 $bfst{$bfls[$_+$blih+1]} if(exists($bfst{$bfls[$_+$blih+1]}));
                                   say    $out8  $z;}
        }
      }
    ...
    for  (0 ..         $#bfls  ){ # setting dirn only seemz to work when not in widt mode so far
      if (-d "$ENV{'HOME'}/bin/$bfls[$_]"){my $dirn= sprintf("%-9s",$bfls[$_].'/');$dirn=~ s/(\/)/$f$1$d/g; # 4m@ && colr dir key
                                   printf($out8 "$d%s"  ,$dirn            );print $out8 $bfst{$bfls[$_        ]} if(exists($bfst{$bfls[$_        ]}));
      }else{
                                   printf($out8 "$h%-9s",$bfls[$_        ]);print $out8 $bfst{$bfls[$_        ]} if(exists($bfst{$bfls[$_        ]}));
      }
                                   say    $out8  $z;}}
  if($sunf){                       say    $out8  $_ for(@ufls);} # try to just show Undescribed File LiSt at end before mAB reorg into 2nd bkgr desc sumb page?
...
sub new{my($clas,$Hfil,$wrnf)=@_;$wrnf=0 unless(defined($wrnf)); # add WaRNingFlag
  ...
  return($self);}
sub a8colr{my $self=shift;return unless(defined($self) && exists($self->{'Hftd'}));
  open my $out8,'>&',STDOUT or die "Can't open  duplic8 STDOUT handle: $!";binmode $out8,':encoding(UTF-8)'; # crE8 local duplic8 of global
...
#     for    (   $gclr = 248;$gclr < 256;$gclr++){$retn.="\e[01;48;5;${gclr}m" . join(':',split(//,$p82k[$gclr]));} $retn.=$z;}
 ...
 $retn.="${G}Lengthy numeric codes remained too obscure, even after much familiarity, so I reordered them logically by approxim8 rainbow order and did a lot of
  searching and sorting among rgb.txt and colors.txt values to construct a new way to represent color palettes. Each color is primarily a single represent8ive
  character, instead of an arcane digit sequence like \"\\e[01;33;44m\" or \"\\x1B[01;38;5;085;48;5;017m\". Also have High intensity F 90..97 and b 100..107.
